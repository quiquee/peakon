...
- (void)centralManager:(CBCentralManager *)central
  ...
  if (frameType == kESSEddystoneTelemetryFrameType) {
    ...
    if ([_delegate respondsToSelector:@selector(beaconScanner:didFindURL: peripheralData:peripheralIdentifier:advertisementData:RSSI:)]) {
        
        NSString *peripheralData = peripheral.name;
        NSString *peripheralIdentifier = [peripheral.identifier UUIDString];
       
        [_delegate beaconScanner:self didFindURL:url peripheralData:peripheralData peripheralIdentifier:peripheralIdentifier advertisementData:advertisementData RSSI:RSSI];
